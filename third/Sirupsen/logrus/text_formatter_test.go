...
func TestFormatting(t *testing.T) {
	...
	for _, tc := range testCases {
		b, _ := Format(WithField("test", tc.value))
...
func TestQuoting(t *testing.T) {
	...
	checkQuoting := func(q bool, value interface{}) {
		b, _ := Format(WithField("test", value))
...
func TestTimestampFormat(t *testing.T) {
	checkTimeStr := func(format string) {
		...
		customStr, _ := Format(WithField("test", "test"))
...
func TestDisableLevelTruncation(t *testing.T) {
	...
	checkDisableTruncation := func(disabled bool, level Level) {
		...
		var b bytes.Buffer
		Level = level
		printColored(&b, entry, keys, nil, timestampFormat)
		logLine := (&b).String()
		if disabled {
			expected := strings.ToUpper(String())
...
func TestDisableTimestampWithColoredOutput(t *testing.T) {
	...
	b, _ := Format(WithField("test", "test"))
...
func TestTextFormatterIsColored(t *testing.T) {
	...
	for _, val := range params {
		t.Run("textformatter_"+val.name, func(subT *testing.T) {
			...
			res := isColored()
			if runtime.GOOS == "windows" && !ForceColors && !val.clicolorForceIsSet {
